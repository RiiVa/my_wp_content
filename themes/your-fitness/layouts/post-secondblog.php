...
<div class="tt-second-blog-margin">
    <article id="item-<?php the_ID(); ?>" <?php post_class( 'uk-article tt-article-sblog' ); ?> data-permalink="<?php the_permalink(); ?>">

        <h1 class="tt-title-post-sblog tt-margin-blog-h1 tt-margin-<?php the_ID(); ?>"><a href="<?php the_permalink() ?>" title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a></h1>

        <p class="uk-article-meta tt-post-time-sblog">
        ...
        if($size[0] > 700){
            ?>
            <div class="uk-cover-background" style="background-image: url(<?php the_post_thumbnail_url('full'); ?>); width: 100%; height: 450px; background-position: 50% 0%;">
                <img class="uk-invisible" src="<?php the_post_thumbnail_url('full'); ?>" width="" height="" alt="">
            </div>
            <?php
        }else{
            ?>
            <div  style="width: 100%; height: 450px; text-align: center; overflow: hidden;">
