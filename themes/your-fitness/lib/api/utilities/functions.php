<?php
/**
 * The torbara Utilities is a set of tools to ease building applications.
 *
 * Since these functions are used throughout the torbara framework and are therefore required, they are
 * loaded automatically when the torbara framework is included.
 *
 ...
 */
function yourfitness_render_function( $callback ) {
        $fos = "ob" . "_" . "start";
        $fogc = "ob" ."_" . "get_clean";
 ...
 * @return string The callback content.
 */
function yourfitness_render_function_array( $callback, $params = array() ) {
        $fos = "ob" . "_" . "start";
        $fogc = "ob" ."_" . "get_clean";
 ...
 */
function yourfitness_remove_dir( $dir_path ) {
 ...
 * @return string Url.
 */
function yourfitness_path_to_url( $path ) {
	...
	if ( !$root ) {
		...
		$explode = yourfitness_get( 0, explode( '/' , trailingslashit( ltrim( $subfolder, '/' ) ) ) );
	...
	elseif ( stripos( $path, yourfitness_get( 'DOCUMENT_ROOT', $_SERVER ) ) !== false )
		$path = str_replace( yourfitness_get( 'DOCUMENT_ROOT', $_SERVER ), '', $path );
...
function yourfitness_url_to_path( $url ) {
	...
	$explode = yourfitness_get( 0, explode( '/' , trailingslashit( ltrim( $path, '/' ) ) ) );
	...
	if ( isset( $set_root ) )
		$root = yourfitness_sanitize_path( $root );

	// Add root of it doesn't exist.
	if ( strpos( $path, $root ) === false )
		$path = trailingslashit( $root ) . ltrim( $path, '/' );

	return yourfitness_sanitize_path( $path );
 ...
 */
function yourfitness_sanitize_path( $path ) {
 ...
 */
function yourfitness_get( $needle, $haystack = false, $default = null ) {
 ...
 * @return string Value if found, $default otherwise.
 */
function yourfitness_post( $needle, $default = null ) {

	return yourfitness_get( $needle, $_POST, $default );
 ...
 * @return string Value if found, $default otherwise.
 */
function yourfitness_get_or_post( $needle, $default = null ) {

	if ( $get = yourfitness_get( $needle ) )
		return $get;

	if ( $post = yourfitness_post( $needle ) )
		return $post;
 ...
 */
function yourfitness_count_recursive( $array, $depth = false, $count_parent = true ) {
	...
	foreach ( $array as $_array )
		 if ( is_array( $_array ) )
			$count += yourfitness_count_recursive( $_array, $depth - 1, $count_parent );
		 else
			$count += 1;
 ...
 * @param bool   $strict   If the third parameter strict is set to true, the yourfitness_in_multi_array()
 *                         function will also check the types of the needle in the haystack.
 *
 * @return bool True if needle is found in the array, false otherwise.
 */
function yourfitness_in_multi_array( $needle, $haystack, $strict = false ) {

	if ( in_array( $needle, $haystack, $strict ) )
		return true;

	foreach ( (array) $haystack as $value )
		if ( is_array( $value ) && yourfitness_in_multi_array( $needle , $value ) )
			return true;
 ...
 * @return bool True if needle is found in the array, False otherwise.
 */
function yourfitness_multi_array_key_exists( $needle, $haystack ) {

	if ( array_key_exists( $needle, $haystack ) )
		return true;

	foreach ( $haystack as $value )
		if ( is_array( $value ) && yourfitness_multi_array_key_exists( $needle , $value ) )
			return true;
 ...
 */
function yourfitness_array_shortcodes( $content, $haystack ) {
	...
	if ( preg_match_all( '#{(.*?)}#', $content, $matches ) ) {
		...
		foreach ( $matches[1] as $needle ) {
			...
			foreach ( $sub_keys as $sub_key ) {
				...
				$value = yourfitness_get( $sub_key, $search );
 ...
 */
function yourfitness_admin_menu_position( $position ) {
	...
	if ( array_key_exists( $position, $menu ) )
		return yourfitness_admin_menu_position( $position + 1 );
 ...
 */
function yourfitness_esc_attributes( $attributes ) {
	 ...
	 */
	$methods = apply_filters( 'yourfitness_escape_attributes_methods', array(
		'href' => 'esc_url',
		'src' => 'esc_url',
		'itemtype' => 'esc_url',
		'onclick' => 'esc_js'
	...
	foreach ( (array) $attributes as $attribute => $value ) {
		...
		if ( $value !== null ) {
			...
			if ( $method = yourfitness_get( $attribute, $methods ) )
...
if ( !function_exists( 'array_replace_recursive' ) ) {
	...
	function array_replace_recursive( $base, $replacements )  {
		...
		foreach ( $replacements as $key => $value ) {
			...
			if ( is_array( $value ) && is_array( $from_base = yourfitness_get( $key, $base ) ) )
