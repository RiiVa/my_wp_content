<?php
/**
 * The torbara Term Meta component extends the torbara Fields and make it easy add fields to any Taxonomy.
 *
 ...
 */
function yourfitness_register_term_meta( array $fields, $taxonomies, $section ) {
	 ...
	 */
	$fields = apply_filters( "yourfitness_term_meta_fields_{$section}", yourfitness_tt_pre_standardize_fields( $fields ) );
	 ...
	 */
	$taxonomies = apply_filters( "yourfitness_term_meta_taxonomies_{$section}", (array) $taxonomies );

	// Stop here if the current page isn't concerned.
	if ( !yourfitness_tt_is_admin_term( $taxonomies ) || !is_admin() )
		return;

	// Stop here if the field can't be registered.
	if ( !yourfitness_register_fields( $fields, 'term_meta', $section ) )
		return false;

	// Load the class only if this function is called to prevent unnecessary memory usage.
        load_template( yourfitness_API_PATH . 'term-meta/class.php', true );

	new yourfitness_tt_Term_Meta( $section );
 ...
 * @ignore
 */
function yourfitness_tt_is_admin_term( $taxonomies ) {

	if ( !$taxonomy = yourfitness_get_or_post( 'taxonomy' ) )
		return false;
