<?php
/**
 * The torbara Templates API allows to load torbara template files as well as loading the entire document.
 *
 ...
 *
 * This function is the root of torbara's framework hierarchy. It must be called from a primary template file
 ...
 * @since 1.0.0
 */
function yourfitness_load_document() {
	 ...
	 * @since 1.0.0
	 */
	do_action( 'yourfitness_before_load_document' );

		/**
		 * Fires when the document loads.
		 *
		 * This hook is the root of torbara's framework hierarchy. It all starts here!
		 *
		 * @since 1.0.0
		 */
		do_action( 'yourfitness_load_document' );
	 ...
	 * @since 1.0.0
	 */
	do_action( 'yourfitness_after_load_document' );

}


/**
 * Load torbara secondary template file.
 *
 * This function loads torbara's default template file. It must be called from a secondary template file
 ...
 * @since 1.0.0
 *
 * @param string $file The filename of the secondary template files. _ _ FILE _ _ is usually to argument to pass.
 *
 * @return bool False if file isn't found.
 */
function yourfitness_load_default_template( $file ) {

    $file = yourfitness_STRUCTURE_PATH . basename( $file );
 ...
 */
function yourfitness_load_fragment_file( $slug ) {
	 ...
	 */
	if ( apply_filters( 'yourfitness_pre_load_fragment_' . $slug, false ) )
		return false;

	// Stop here if fragment file doesn't exists.
	if ( !file_exists( yourfitness_FRAGMENTS_PATH . $slug . '.php' ) )
		return false;

        load_template( yourfitness_FRAGMENTS_PATH . $slug . '.php', true );
 ...
 * @since 1.0.0
 */
function yourfitness_comment_callback( $comment, $args, $depth ) {
