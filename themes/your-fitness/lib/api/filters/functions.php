<?php
/**
 * torbara Filters extends WordPress Filters by registering sub filters if it is told to do so.
 *
 * @package API\Filters
 */
 ...
 */
function yourfitness_add_filter( $id, $callback, $priority = 10, $args = 1 ) {

	if ( is_callable( $callback ) )
		return add_filter( $id, $callback, $priority, $args );

	return yourfitness_tt_add_anonymous_filter( $id, $callback, $priority, $args );
 ...
 */
function yourfitness_apply_filters( $id, $value ) {
 ...
 *                  return value.
 */
function yourfitness_has_filters( $id, $callback = false ) {
 ...
 * @ignore
 */
function yourfitness_tt_add_anonymous_filter( $id, $callback, $priority = 10, $args = 1 ) {

        load_template( yourfitness_API_PATH . 'filters/class.php', true );

	new yourfitness_tt_Anonymous_Filters( $id, $callback, $priority, $args );
