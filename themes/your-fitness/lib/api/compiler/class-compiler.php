 ...
 */
final class yourfitness_tt_Compiler {
	...
	public function __construct( $args ) {
		...
		$this->dir = yourfitness_get_compiler_dir( is_admin() ) . $this->compiler['id'];
		$this->url = yourfitness_get_compiler_url( is_admin() ) . $this->compiler['id'];
		...
		$this->enqueue_file();
                if ( file_exists( get_template_directory().'/css/theme.css' ) ) {
                    wp_enqueue_style('torbara-lib-style', get_template_directory_uri() . '/css/theme.css');
                }
	...
	public function set_fragments() {
		...
		if ( $added_fragments = yourfitness_get( $this->compiler['id'], $yourfitness_tt_compiler_added_fragments[$this->compiler['format']] ) )
		 ...
		 */
		$this->compiler['fragments'] = apply_filters( 'yourfitness_compiler_fragments_' . $this->compiler['id'], $this->compiler['fragments'] );
	...
	public function set_filname() {
		...
		if ( !yourfitness_tt_is_compiler_dev_mode() || !@is_dir( $this->dir ) )
		...
		foreach ( $this->compiler['fragments'] as $id => $fragment ) {
			...
			if ( strpos( $fragment, $_SERVER['HTTP_HOST'] ) !== false || preg_match( '#^\/[^\/]#', $fragment ) == true )
				$fragments_filemtime[$id] = @filemtime( yourfitness_url_to_path( $fragment ) );
	...
	public function cache_file_exist() {
		...
		if ( ( $filname = yourfitness_get( 'filename', $this->compiler ) ) && file_exists( $this->dir . '/' . $filname ) )
			return true;
	...
	public function get_url() {
		...
		$url = trailingslashit( $this->url ) . yourfitness_get( 'filename', $this->compiler );
	...
	public function get_internal_content() {
		...
		if ( !file_exists( $fragment ) ) {
			...
			$fragment = yourfitness_url_to_path( $fragment );
	...
	public function add_content_media_query( $content ) {
		...
		if ( !( $query = yourfitness_get( 'query', $parse_url ) ) || stripos( $query, 'yourfitness_compiler_media_query' ) === false )
			return $content;

		// Wrap the content in the query.
		$new_content = '@media ' . yourfitness_get( 'yourfitness_compiler_media_query', wp_parse_args( $query ) ) . ' {' . "\n";
	...
	public function format_content( $content ) {
		...
		if ( $this->compiler['type'] == 'style' ) {
			...
			if ( $this->compiler['format'] == 'less' ) {
				...
				if ( !class_exists( 'yourfitness_Lessc' ) ){
                                    load_template( yourfitness_API_PATH . 'compiler/vendors/lessc.php', true );
                                }

				$less = new yourfitness_Lessc();

				$content = $less->compile( $content );

			}

			if ( !yourfitness_tt_is_compiler_dev_mode() )
				$content = $this->strip_whitespace( $content );

		}

		if ( $this->compiler['type'] == 'script' && !yourfitness_tt_is_compiler_dev_mode() && $this->compiler['minify_js'] ) {

                    if ( !class_exists( 'JSMin' ) ){
                        load_template( yourfitness_API_PATH . 'compiler/vendors/js-minifier.php', true );
                    }
	...
	public function css_path_to_url( $matches, $base_is_path = false ) {
            ...
            // Rebuild url and make sure it is a valid one using the yourfitness_path_to_url function.
            $url = yourfitness_path_to_url( trailingslashit( $base ) . $rebuilt_path );
	...
	public function kill() {
		...
		// Send report if set.
		if ( yourfitness_get( 'yourfitness_send_compiler_report' ) )
			$this->report();

		$html = yourfitness_output( 'yourfitness_compiler_error_title_text', sprintf(
			'<h2>%s</h2>',
			esc_html__( 'Not cool, torbara cannot work its magic :(', 'your-fitness' )
		) );

		$html .= yourfitness_output( 'yourfitness_compiler_error_message_text', sprintf(
			'<p>%s</p>',
			esc_html__( 'Your current install or file permission prevents torbara from working its magic. Please get in touch with torbara support, we will gladly get you started within 24 - 48 hours (working days).', 'your-fitness' )
		) );

		$html .= yourfitness_output( 'yourfitness_compiler_error_contact_text', sprintf(
			'<a class="button" href="http://www.gettorbara.io/contact/?compiler_report=1" target="_blanc">%s</a>',
			esc_html__( 'Contact torbara Support', 'your-fitness' )
		) );

		$html .= yourfitness_output( 'yourfitness_compiler_error_report_text', sprintf(
			'<p style  =  "margin-top: 12px; font-size: 12px;"><a href="' . add_query_arg( 'yourfitness_send_compiler_report', true ) . '">%1$s</a>. %2$s</p>',
			esc_html__( 'Send us an automatic report', 'your-fitness' ),
			esc_html__( 'We respect your time and understand you might not be able to contact us.', 'your-fitness' )
		) );
