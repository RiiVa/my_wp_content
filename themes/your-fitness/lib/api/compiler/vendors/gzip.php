...
if ( !isset( $_GET['torbara-compiler'] ) && $_GET['torbara-compiler'] != 'cached' ){
    return;
}	
...
if ( ( $id = $_GET['id'] ) && ( $file = $_GET['file'] ) && ( $type = $_GET['type'] ) ) {
	...
	$file = realpath( get_template_directory().'/lib/api/compiler/vendors' . DIRECTORY_SEPARATOR . $id . DIRECTORY_SEPARATOR . $file );
