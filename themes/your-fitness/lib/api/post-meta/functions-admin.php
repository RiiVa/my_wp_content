<?php
/**
 * The torbara Post Meta component extends the torbara Fields and make it easy add fields to any Post Type.
 *
 ...
 */
function yourfitness_register_post_meta( array $fields, $conditions, $section, $args = array() ) {

	global $yourfitness_tt_post_meta_conditions;
	 ...
	 */
	$fields = apply_filters( "yourfitness_post_meta_fields_{$section}", yourfitness_tt_pre_standardize_fields( $fields ) );
	 ...
	 */
	$conditions = apply_filters( "yourfitness_post_meta_post_types_{$section}", $conditions );
	$yourfitness_tt_post_meta_conditions = array_merge( $yourfitness_tt_post_meta_conditions, (array) $conditions );

	// Stop here if the current page isn't concerned.
	if ( !yourfitness_tt_is_post_meta_conditions( $conditions ) || !is_admin() )
		return;

	// Stop here if the field can't be registered.
	if ( !yourfitness_register_fields( $fields, 'post_meta', $section ) )
		return false;

	// Load the class only if this function is called to prevent unnecessary memory usage.
        load_template( yourfitness_API_PATH . 'post-meta/class.php', true );

	new yourfitness_tt_Post_Meta( $section, $args );
 ...
 * @ignore
 */
function yourfitness_tt_is_post_meta_conditions( $conditions ) {

	// Check if it is a new post and treat it as such.
	if ( stripos( $_SERVER['REQUEST_URI'], 'post-new.php' ) !== false ) {

		if ( !$current_post_type = yourfitness_get( 'post_type' ) )
		...
		if ( $id = yourfitness_get( 'post' ) )
			$post_id = $id;
		// Try to get id from $_POST.
		elseif ( $id = yourfitness_post( 'post_ID' ) )
			$post_id = $id;
...
add_action( 'admin_print_footer_scripts', 'yourfitness_tt_post_meta_page_template_reload' );
 ...
 * @ignore
 */
function yourfitness_tt_post_meta_page_template_reload() {

	global $yourfitness_tt_post_meta_conditions, $pagenow;

	// Stop here if not editing a post object.
	if ( !in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) )
		return;

	$encode = json_encode( $yourfitness_tt_post_meta_conditions );

	// Stop here of there isn't any post meta assigned to page templates.
	if ( stripos( $encode, '.php' ) === false )
		return;

	echo "<scr"."ipt type='text/jav"."ascript'>\n!(function(a){a(document).ready(function(){a('#page_template').data('torbara-pre',a('#page_template').val());a('#page_template').change(function(){if(a.inArray(a(this).val(),$encode)===-1&&a.inArray(a(this).data('torbara-pre'),$encode)===-1){return}a(this).data('torbara-pre',a(this).val());var b=a('#save-action #save-post');if(b.length===0){b=a('#publishing-action #publish')}b.trigger('click');a('#wpbody-content').fadeOut()})})})(jQuery);\n</scr"."ipt>";
...
if ( !isset( $yourfitness_tt_post_meta_conditions ) )
	$yourfitness_tt_post_meta_conditions = array();