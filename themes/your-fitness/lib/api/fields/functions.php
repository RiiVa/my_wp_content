<?php
/**
 * The torbara Fields component offers a range of fields which can be used in the WordPress admin.
 *
 ...
 */
function yourfitness_register_fields( array $fields, $context, $section ) {

	if ( empty( $fields ) )
		return false;

	// Load the class only if this function is called to prevent unnecessary memory usage.
        load_template( yourfitness_API_PATH . 'fields/class.php', true );

	$class = new yourfitness_tt_Fields();
 ...
 */
function yourfitness_get_fields( $context, $section = false ) {

	if ( !class_exists( 'yourfitness_tt_Fields' ) )
		return;
 ...
 * {@see yourfitness_get_fields()}.
 *
 * @since 1.0.0
 *
 * @param array $field Array of data obtained using {@see yourfitness_get_fields()}.
 */
function yourfitness_field( $field ) {
 ...
 * @ignore
 */
function yourfitness_tt_pre_standardize_fields( $fields ) {
	...
	foreach ( $fields as $field ) {
		...
		if ( yourfitness_get( 'type', $field ) === 'group' )
			$_fields[$field['id']]['fields'] = yourfitness_tt_pre_standardize_fields( $field['fields'] );
