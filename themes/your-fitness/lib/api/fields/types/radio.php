...
yourfitness_add_smart_action( 'yourfitness_field_radio', 'yourfitness_field_radio' );
 ...
 * }
 */
function yourfitness_field_radio( $field ) {
	...
	<fieldset>
		<?php $i = 0; foreach ( $field['options'] as $id => $radio ) :
			...
			$has_image = in_array( yourfitness_get( 'extension', pathinfo( $radio ) ), $extensions ) ? 'bs-has-image' : false;
			...
			<label class="<?php echo esc_attr( $has_image ); ?>">
				...
				<?php endif; ?>
				<input type="radio" name="<?php echo esc_attr( $field['name'] ); ?>" value="<?php echo esc_attr( $id ); ?>" <?php checked( $id, $field['value'], 1 ); ?> <?php echo yourfitness_esc_attributes( $field['attributes'] ); ?>/>
				<?php   if (!$has_image) {
                                            echo esc_url($radio);
                                        }
                                        ?>
