<?php
/*
 * Plugin Name: HooksOcean
 * Plugin URI: http://themeforest.net/user/torbara/?ref=torbara
 * Description: HooksOcean
 * Author: Nemirovskiy Vitaliy
 * Author URI: http://themeforest.net/user/torbara/portfolio/?ref=torbara
 * Author e-mail: nemirovskiyvitaliy@gmail.com
 * Version: 1.0.0
 * Date: 10.11.2016
 * License: GPL2+
 */



function hooksocean_add_admin_pages(){
    add_options_page('Hooks Ocean', 'Hooks Ocean', 'manage_options', 'hooksocean', 'hooksocean_options_page');
}

function hooksocean_options_page(){
    echo "<h1>Hooks Ocean</h1>";

    /*
     *  add option Hooks Ocean
     */
    add_option('hooksocean_auto_p', 'No');
    add_option('hooksocean_admin_bar', 'No');

    hooksocean_change_options();

}

function hooksocean_change_options(){

    if (isset($_POST['hooksocean_change_options_btn'])){

        $hooksocean_auto_p    = $_POST['hooksocean_auto_p'];
        $hooksocean_admin_bar = $_POST['hooksocean_admin_bar'];

        update_option('hooksocean_auto_p', $hooksocean_auto_p);
        update_option('hooksocean_admin_bar', $hooksocean_admin_bar);

    }


    echo "<h3 style='color: red;'>No or Yes</h3>";

    echo "<form name='hooksocean_change_options' method='post' action='".$_SERVER['PHP_SELF']."?page=hooksocean&amp;updated=true'>";
        echo "<table>";
            echo "<tr>";
                echo "<td>Disable AUTO P?</td>";
                echo "<td>";
                    echo "<input type='text' name='hooksocean_auto_p' value='".get_option('hooksocean_auto_p')."'/>";
                echo "</td>";
            echo "</tr>";

            echo "<tr>";
                echo "<td>Disable Admin Bar?</td>";
                echo "<td>";
                    echo "<input type='text' name='hooksocean_admin_bar' value='".get_option('hooksocean_admin_bar')."'/>";
                echo "</td>";
            echo "</tr>";

            echo "<tr>";
                echo "<td>";
                    echo "<input type='submit' name='hooksocean_change_options_btn' value='SAVE'>";
                echo "</td>";
            echo  "</tr>";
        echo "</table>";
    echo "</form>";
}

function hooksocean_run(){

}

    /*
     *  Disable Admin Bar
     */
    if ( get_option('hooksocean_admin_bar') == 'yes' or get_option('hooksocean_admin_bar') == 'Yes'){
        add_filter( 'show_admin_bar', '__return_false' );
    }

    /*
     *  Disable AUTO P
     */
    if ( get_option('hooksocean_auto_p')  == 'yes' or get_option('hooksocean_auto_p')  == 'Yes'){
        remove_filter('the_content', 'wpautop');
        remove_filter('the_excerpt', 'wpautop');

    }


add_action('admin_menu', 'hooksocean_add_admin_pages');
add_action( 'admin_init', 'hooksocean_run' );


// Additional links in plugin settings: Torbara.com | Portfolio
$tt_f_plugin_links = function($links) {
    array_push($links, '<a title="We are developing beautiful themes and applications for the web!" href="http://torbara.com" target="_blank" style="font-weight: bold; font-size: 14px;"><img src="data:image/svg+xml;base64," alt="" style="width: 24px; vertical-align: middle; position: relative; top: -1px;"> Torbara.com</a>');
    array_push($links, '<a title="Our portfolio on Envato Market" href="http://themeforest.net/user/torbara/portfolio?ref=torbara" target="_blank"><img src="data:image/svg+xml;utf8;base64," alt="" style="width: 16px; vertical-align: middle; position: relative; top: -2px;"> Portfolio</a>');
    return $links;
};
add_filter( "plugin_action_links_".plugin_basename( __FILE__ ), $tt_f_plugin_links );