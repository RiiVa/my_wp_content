...
<div id="widgets">
        ...
        <table data-position="<?php echo $posname ?>" class="uk-table uk-table-hover uk-table-middle tm-table">
            ...
            <?php
                ...
                foreach ($widgets[$posname] as $widget) {
                    $w_shortcode = '[tt_widget id="'.$widget->id.'"]';
                    $html[] = '<tr data-widget-name="'.(isset($widget->params['title']) && $widget->params['title'] ? $widget->params['title'] : $widget->name).'">';
                    $html[] = '<td>'
                            . '    <div class="uk-text-truncate">'.(isset($widget->params['title']) && $widget->params['title'] ? $widget->params['title'] : $widget->name).'</div>'
                            . '    <div class="uk-text-small uk-text-muted">'
                            . '        Shortcode: <i class="uk-icon-code" onclick="tt_copyToClipboard(\''.esc_attr($w_shortcode).'\')" title="Copy shortcode: '.esc_attr($w_shortcode).'"></i>'
                            . '    </div>'
                            . '</td>';
    ...
    <?php require_once __DIR__.'/awesome_assignment.php'; ?>
