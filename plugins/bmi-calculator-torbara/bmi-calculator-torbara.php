<?php
/*
 * Plugin Name: BMI Calculator Torbara
 * Plugin URI: http://themeforest.net/user/torbara/?ref=torbara
 * Description: BMI Calculator Torbara
 * Author: Nemirovskiy Vitaliy
 * Author URI: http://themeforest.net/user/torbara/portfolio/?ref=torbara
 * Author e-mail: nemirovskiyvitaliy@gmail.com
 * Version: 1.0.0
 * Date: 04.04.2016
 * License: GPL2+
 */

class BMICalculatorTorbara extends WP_Widget {

    function __construct() {
        parent::__construct("bmi_calculator_torbara",__('BMI Calculator Torbara', 'torbara-plugins'), array("description" => "BMI Calculator Torbara"));
    }
    
   
    
    
    
    
    function update($new_instance, $old_instance) {
        $instance = array();
        $instance['title'] = (!empty($new_instance['title']) ) ? strip_tags($new_instance['title']) : '';
        $instance['gender_sex'] = $new_instance['gender_sex'];
        $instance['age'] = $new_instance['age'];
        $instance['link_article'] = $new_instance['link_article'];
        $instance['consultation_url'] = $new_instance['consultation_url'];
        $instance['start_year'] =  $new_instance['start_year'];
        $instance['final_year_int'] = $new_instance['final_year_int'];
        $instance['start_weight'] = $new_instance['start_weight'];
        $instance['final_weight'] = $new_instance['final_weight'];
        $instance['start_height'] = $new_instance['start_height'];
        $instance['final_growth'] = $new_instance['final_growth'];
        return $instance;
    }

    function widget($args, $instance) {
        
        extract($args, EXTR_SKIP);
        echo $before_widget;
        
        $title = apply_filters('bmi_calculator_torbara', $instance['title']);
        $gender_sex = $instance['gender_sex'] ? true : false;
        $age = $instance['age'] ? true : false;
        
        if (!empty($title)) { echo $before_title . $title . $after_title; } 
        
        $title = $instance["title"];
        
        $start_year     = $instance['start_year']; //year
        
        $start_weight   = $instance['start_weight'];   //kg
        $start_height   = $instance['start_height'];  // cm
        
        $sex            = $gender_sex;
        $year_of_birth  = $age;
        $article_BMI    = $instance['link_article'];
        $consultation_url    = $instance['consultation_url'];
        
        
        $final_year_int = $instance['final_year_int'];
        $for_i_year     = $final_year_int - $start_year;
        
        $final_weight   = $instance['final_weight'];
        $for_i_weight   = $final_weight - $start_weight;
        
        $final_growth   = $instance['final_growth'];
        $for_i_rise     =  ($final_growth - $start_height) / 2;
        
       
?>
<script>
var consultation_url = "<?php echo $consultation_url; ?>";
</script>

<div class="uk-grid uk-container-center uk-flex-center" style="margin: 0px;">
<div class="uk-width-medium-2-10 uk-hidden-small" style="background-image: url(/wp-content/themes/your-fitness/images/bmi/background-bg-5.jpg);
    background-repeat: no-repeat;
    background-size: contain;
    background-position: bottom;">
</div>
<div class="uk-grid-width-medium-4-10 uk-grid-width-small-4-10 uk-grid-width-large-4-10 uk-grid-width-xlarge-4-10" >
        <div class="tt-border-form">
    <form>
  
        <h3 class="header-text">BMI calculator</h3>
        <a href="<?php echo $article_BMI; ?>" target="_blank" style="text-decoration: none;">
        <h4 class="bmi-link">all about BMI</h4>
        </a>
<?php 
                                       
    if ($sex == "1"){
        echo '
                    
                    <input type="radio" id="r1"  name="sex" value="1" checked="checked"/> 
                    <label for="r1" style="float: left; margin-right: 60px;">
                    <span style="margin-right: 14px;"></span><p style="font-family: \'Roboto\'; font-weight: 100; font-size: 19px; margin-top: 4px; float: right;">Female</p></label>

                    <input type="radio" id="r2" name="sex" value="2" style="float: left;"/>
                    <label for="r2" style="float: left;">
                    <span style="margin-right: 14px;"></span><p style="font-family: \'Roboto\'; font-weight: 100; font-size: 19px; margin-top: 4px; float: right;">Male</p></label>
                    ';
    }
       
    if ($year_of_birth == "1"){     
        echo '<select name="year_birthday" class="select year_birthday">
              <option value="0">Birthday</option>';

        for ($i = 0; $i <= $for_i_year; $i++){
            $result = $start_year +$i;
            echo '<option value="'.$result.'">'.$result.'</option>';
        }
        echo '</select>';
    }

    echo '
    <div class="tt-box-value">
    <select name="weight" class="select_to weight-bmi"> 
          <option value="0">Weight</option>'; 
        
        for ($i = 0; $i <= $for_i_weight; $i++){
            $result = $start_weight +$i;
            echo '<option value="'.$result.'">'.$result.' kg</option>';
        }

    echo  '</select>
           <select name="height" class="select_to"> 
           <option value="0">Height</option>';
    
        for ($i = 0; $i <= $for_i_rise; $i++){
            $result = $start_height + $i + $i;
            echo '<option value="'.$result.'">'.$result.' cm</option>';
        }
?>
        </select> </div>
        <input type="submit" class="ak-bmicalc-submit" value="calculate"/>
    </form>
    
   </div>

</div>
<div class="ak-bmi-result uk-grid-width-medium-4-10" style="padding-bottom: 25px;"></div>
</div> 
        
        <?php
         
    }

    function form($instance) {
        $title= " ";
        $instance['gender_sex'] = " ";
        $instance["title"] = " ";
        $instance['age'] = " ";
        
        $tableId = $this->get_field_id("title");
        $tableName = $this->get_field_name("title");
        echo '<label for="' . $tableId . '">Title</label><br>';
        echo '<input id="' . $tableId . '" type="text" name="' .
        $tableName . '" value="' . $title . '"><br>';
        
        
        $gender_sex = isset( $instance['gender_sex'] ) ? $instance['gender_sex'] : false; ?>
          
        <p>
            <input type="checkbox" id="<?php echo $this->get_field_id('gender_sex'); ?>" name="<?php echo $this->get_field_name('gender_sex'); ?>" <?php if ($instance['gender_sex']) echo 'checked' ?> />
            <label for="<?php echo $this->get_field_id('gender_sex'); ?>"><?php esc_html_e('Use gender in calculation', 'torbara-plugins'); ?></label>
        </p>
        
        
       <?php $age = isset( $instance['age'] ) ? $instance['age'] : false; ?>
          
        <p>
            <input type="checkbox" id="<?php echo $this->get_field_id('age'); ?>" name="<?php echo $this->get_field_name('age'); ?>" <?php if ($instance['age']) echo 'checked' ?> />
            <label for="<?php echo $this->get_field_id('age'); ?>"><?php esc_html_e('Use age in calculation', 'torbara-plugins'); ?></label>
        </p>
        
        
        <?php $link_article = isset( $instance['link_article']) ? esc_attr( $instance['link_article'] ) : ''; ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('link_article')); ?>">
                Link to the article BMI: 
                <input class="widefat" 
                        id="<?php echo esc_attr($this->get_field_id('link_article')); ?>" 
                        name="<?php echo esc_attr($this->get_field_name('link_article')); ?>" 
                        type="text" 
                        value="<?php echo esc_attr($link_article); ?>" />
            </label>
        </p>
        
        
        <?php $consultation_url = isset( $instance['consultation_url']) ? esc_attr( $instance['consultation_url'] ) : ''; ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('consultation_url')); ?>">
                Get Free Consultation: 
                <input class="widefat" 
                        id="<?php echo esc_attr($this->get_field_id('consultation_url')); ?>" 
                        name="<?php echo esc_attr($this->get_field_name('consultation_url')); ?>" 
                        type="text" 
                        value="<?php echo esc_attr($consultation_url); ?>" />
            </label>
        </p>
        
        
        <?php $start_year = isset($instance['start_year']) ? esc_attr( $instance['start_year'] ) : ''; ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('start_year')); ?>">
                Start with the year of birth: 
                <input class="widefat" 
                        id="<?php echo esc_attr($this->get_field_id('start_year')); ?>" 
                        name="<?php echo esc_attr($this->get_field_name('start_year')); ?>" 
                        type="text" 
                        value="<?php echo esc_attr($start_year); ?>" />
            </label>
        </p>
        
        <?php $final_year_int = isset($instance['final_year_int']) ? esc_attr( $instance['final_year_int'] ) : ''; ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('final_year_int')); ?>">
                The final year of birth: 
                <input class="widefat" 
                        id="<?php echo esc_attr($this->get_field_id('final_year_int')); ?>" 
                        name="<?php echo esc_attr($this->get_field_name('final_year_int')); ?>" 
                        type="text" 
                        value="<?php echo esc_attr($final_year_int); ?>" />
            </label>
        </p>
        
        
        <?php $start_weight = isset($instance['start_weight']) ? esc_attr( $instance['start_weight'] ) : ''; ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('start_weight')); ?>">
                Starting weight: 
                <input class="widefat" 
                        id="<?php echo esc_attr($this->get_field_id('start_weight')); ?>" 
                        name="<?php echo esc_attr($this->get_field_name('start_weight')); ?>" 
                        type="text" 
                        value="<?php echo esc_attr($start_weight); ?>" />
            </label>
        </p>


        <?php $final_weight = isset($instance['final_weight']) ? esc_attr( $instance['final_weight'] ) : ''; ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('final_weight')); ?>">
                Final weight: 
                <input class="widefat" 
                        id="<?php echo esc_attr($this->get_field_id('final_weight')); ?>" 
                        name="<?php echo esc_attr($this->get_field_name('final_weight')); ?>" 
                        type="text" 
                        value="<?php echo esc_attr($final_weight); ?>" />
            </label>
        </p>
        
        <?php $start_height = isset($instance['start_height']) ? esc_attr( $instance['start_height'] ) : ''; ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('start_height')); ?>">
                The initial height: 
                <input class="widefat" 
                        id="<?php echo esc_attr($this->get_field_id('start_height')); ?>" 
                        name="<?php echo esc_attr($this->get_field_name('start_height')); ?>" 
                        type="text" 
                        value="<?php echo esc_attr($start_height); ?>" />
            </label>
        </p>
        
        <?php $final_growth = isset($instance['final_growth']) ? esc_attr( $instance['final_growth'] ) : ''; ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('final_growth')); ?>">
                The final height: 
                <input class="widefat" 
                        id="<?php echo esc_attr($this->get_field_id('final_growth')); ?>" 
                        name="<?php echo esc_attr($this->get_field_name('final_growth')); ?>" 
                        type="text" 
                        value="<?php echo esc_attr($final_growth); ?>" />
            </label>
        </p>
        
        <?php
       
    }

}


function bmi_script() {
    wp_enqueue_script( 'bmi_script', get_site_url() . '/wp-content/plugins/bmi-calculator-torbara/js/bmi_script.js', array( 'jquery' ) );
}

add_action('wp_enqueue_scripts', 'bmi_script' );    

add_action("widgets_init", function () {
    register_widget("BMICalculatorTorbara");
});

// Additional links in plugin settings: Torbara.com | Portfolio
$tt_f_plugin_links = function($links) {
    array_push($links, '<a title="We are developing beautiful themes and applications for the web!" href="http://torbara.com" target="_blank" style="font-weight: bold; font-size: 14px;"><img src="data:image/svg+xml;base64," alt="" style="width: 24px; vertical-align: middle; position: relative; top: -1px;"> Torbara.com</a>');
    array_push($links, '<a title="Our portfolio on Envato Market" href="http://themeforest.net/user/torbara/portfolio?ref=torbara" target="_blank"><img src="data:image/svg+xml;utf8;base64," alt="" style="width: 16px; vertical-align: middle; position: relative; top: -2px;"> Portfolio</a>');
    return $links;
};
add_filter( "plugin_action_links_".plugin_basename( __FILE__ ), $tt_f_plugin_links );